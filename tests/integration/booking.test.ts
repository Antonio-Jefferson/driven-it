...
describe('PUT /booking', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 404 with a invalid body - theres no roomId', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
    ...
    it('should respond with status 403 with a invalid body - theres not vacancy', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
    ...
    it('should respond with status 404 when user has not a booking ', async () => {
      ...
      const userBooking = await createBooking({ userId: otherUser.id, roomId: room.id });

      const response = await server.put(`/booking/${userBooking.id}`).set('Authorization', `Bearer ${token}`).send({
        roomId: room.id,
      });
